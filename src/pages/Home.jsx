...
const Home = () => {
  return (
    ...
    {/* About Us Section */}
<div
  className="relative w-full bg-cover bg-center py-20 px-4 sm:px-8 md:px-16"
  style={{ backgroundImage: `url(${aboutUs})` }}
>
  <div className="bg-white bg-opacity-80 p-6 sm:p-8 md:p-10 rounded-lg w-full max-w-3xl ml-0 md:ml-10">
    <h1 className="text-3xl sm:text-4xl font-bold mb-6 bg-black text-white inline-block px-4 py-2 rounded">
      ADOUT US 
    </h1>
    ...
    <h2 className="text-2xl sm:text-3xl font-semibold mb-4 mt-8 text-left">Our Vision</h2>
    <p className="text-base sm:text-lg mb-6 text-left">
    ...
    <h2 className="text-2xl sm:text-3xl font-semibold mb-4 mt-8 text-left">Industry Wide Success</h2>
    <p className="text-base sm:text-lg mb-4 text-left">
      We work with a large number of clients from various industries, including:
    </p>

    <ul className="list-disc list-inside text-base sm:text-lg mb-6 max-w-md text-left">
      <li>Real Estate</li>
    ...
    {images.map((img, index) => (
      <SwiperSlide key={index} className="flex justify-center">
        <img
          src={img}
          alt={`Slide ${index + 1}`}
          className="w-[160px] h-[240px] object-contain rounded-lg"
          loading="lazy"
        />
      </SwiperSlide>
    ))}
